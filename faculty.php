...
try {
    $db = get_db();
    $stmt = $db->query("SELECT `image_path`, `name`, `designation`, `qualification` FROM `faculty` WHERE `is_active` = 1 ORDER BY `display_order` ASC, `id` ASC");
    $dbStaff = $stmt->fetchAll();
    $staff = [];
    foreach ($dbStaff as $ds) {
        $staff[] = [$ds['image_path'], $ds['name'], $ds['designation'], $ds['qualification']];
    }
} catch (Exception $e) {
    $staff = [];
}
